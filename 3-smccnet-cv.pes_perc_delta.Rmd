...
#CCcoef <- c(1, 1, 2)    # Weighted version of SmCCNet - Increased weight on omic/phenotype
CCcoef <- c(1, 1, 5)    # Weighted version of SmCCNet - Increased weight on omic/phenotype
...
# Set a CV directory
#cv_dir <- "./results/SmCCNet-5FoldCV-percPES-Unweighted/"
#cv_dir <- "./results/SmCCNet-5FoldCV-percPES-Weighted-1-1-2/"
cv_dir <- "./results/SmCCNet-5FoldCV-percPES-Weighted-1-1-5/"
dir.create(cv_dir)
...
cv_dirs <- c(
  "./results/SmCCNet-5FoldCV-percPES-Unweighted/", 
  "./results/SmCCNet-5FoldCV-percPES-Weighted-1-1-2/",
  "./results/SmCCNet-5FoldCV-percPES-Weighted-1-1-5/")

# Scaling constants matching each CV dir
scaling_constants <- c("(1, 1, 1)", "(1, 2, 1)", "(1, 5, 1)")
...
for (i in seq(1, length(cv_dirs))) {
  ...
  for (m in seq(1, length(modules))) {
    ...
    correlation_df <- data.frame(
      ...
      "var_exp" = round(pca_importance[2, 1], 3),
...
col.names = c("Scaling Weights\n(a, b, c)", "Network", "Pheno Corr\n(P-Value)", "PC1 Var Exp", "Taxa", "Proteins", "Nodes")
supp_table1 <- kbl(subnetwork_summary, booktabs = TRUE, align = "c", col.names = col.names) %>%
  collapse_rows(c(1, 2), valign = "middle") %>%
  row_spec(c(1:10, 21:28), background = "#EDEDED") %>%
  kable_styling(font_size = 20) 
show(supp_table1)

# Save table
supp_table1 %>% save_kable(file = "figures-and-tables/PExS_perc_delta/supp_table_1.perc_delta.png", zoom = 0.75)
```
